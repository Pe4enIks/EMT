...
from utils import make_exp_dirs, mkdir_and_rename, parse_options
...
def create_train_val_dataloader(opt, logger):
    ...
    for phase, dataset_opt in opt["datasets"].items():
        ...
        if phase == "train":
            dataset_enlarge_ratio = dataset_opt.get(
        ...
        while train_data is not None:
            ...
            if opt.get("val") is not None and (
                ...
                for val_loader in val_loaders:
                    model.validation(
                        val_loader, current_iter, tb_logger, opt["val"]["save_val_img"]
                    )
